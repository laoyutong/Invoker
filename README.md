...
- **长期记忆文件** — 对话独有的长期信息保存到 `.memory/index.json` 与 `.memory/items/<id>.json`
- **自动加载** — 新会话启动后自动读取记忆，并作为动态 prompt section 注入
- **自动提取** — 成功对话后由 LLM 从最近对话中提取候选记忆，失败不影响主流程
- **严格边界** — 只保存无法从代码、git、README、文档、工具输出推导的信息
- **记忆类型** — `user_profile` 用户画像 / `behavior_feedback` 行为反馈 / `project_dynamic` 项目动态 / `external_resource` 外部资源
- **安全过滤** — 默认拒绝低置信度内容、敏感凭据、仓库事实、一次性任务进展
- **硬性上限** — 最多 80 条记忆、索引最多 200 条、单个 item 文件最多 4000 字符、注入 prompt 最多 4000 字符
...
|------|------|
| `/memory` | 查看当前跨会话记忆 |
| `/memory on` | 开启本次运行的记忆注入与写入 |
| `/memory off` | 关闭本次运行的记忆注入与写入 |
| `/memory extract` | 手动从当前会话提取一次 |
